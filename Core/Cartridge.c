...
// HasHeader
// ----------------------------------------------------------------------------
static uint cartridge_HasHeader(const byte* header) {
  uint index;
...
static void cartridge_ReadHeader(const byte* header) {
  ...
  cartridge_pokey = (header[54] & 1)? 1: 0;
...
// Load
// ----------------------------------------------------------------------------
static uint _cartridge_Load(const byte* data, uint size) {
  uint index;
  if(size <= 128) {
#if 0
...
// Load
// ----------------------------------------------------------------------------
uint cartridge_Load(char *filename) {
...
uint cartridge_Load_buffer(char* rom_buffer, int rom_size) {
  cartridge_Release();
...
// IsLoaded
// ----------------------------------------------------------------------------
uint cartridge_IsLoaded( ) {
  return (cartridge_buffer != NULL)? 1: 0;
}
